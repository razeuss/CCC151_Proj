...
public class ClassRepControl {
    ...
    public void initialize(String user_id_number, String academic_year) {
        ...
        String user_full_name = user.getFirst_name() + " " + user.getMiddle_name() + " " + user.getLast_name() + " " + user.getSuffix_name();
    ...
    private ObservableList<StudentPaymentInfo> getPayersList() {
        ...
        try {
            ...
            String members_data_query = "SELECT `id_number`, `first_name`,`middle_name`, `last_name`, `suffix_name`, `year_level` "
                    ...
                    + "WHERE m.`organization_code` = \"" + org_code.getText() + "\" " + "AND s.`program_code` = \"" + user.getProgram_code()
            ...
            while (result.next()) {
                ...
                while (result_2.next()){
                    String status = result_2.getString("status");
    ...
    private void searchID() {
        if (!search_id.getText().isEmpty()) {
            ...
            ObservableList<StudentPaymentInfo> member_list = FXCollections.observableArrayList();
            try {
                String search_id_query = "SELECT `id_number`, `first_name`,`middle_name`, `last_name`, `year_level` "
                        + "FROM `members` AS m LEFT JOIN `students` AS s ON m.`member_id` = s.`id_number` "
