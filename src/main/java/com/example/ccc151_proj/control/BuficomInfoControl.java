...
public class BuficomInfoControl {
    ...
    @FXML
    private Button add_contribution_button;
    @FXML
    ...
    public String getOrg_Code(){
        return this.org_code_textfield.getText();
    }
    ...
    private void verify_payments_clicked(){
        ...
        add_contribution_button.setDisable(false);
        ObservableList<Node> children = parent.getChildren();
        parent.getChildren().remove((AnchorPane) children.get(1));
        try {
            ...
            dashboard_control.initialize(this.getOrg_Code(), LoginProcess.getAcademic_year());
    ...
    private void students_records_clicked(){
        ...
        parent.getChildren().remove((AnchorPane) children.get(1));
        try {
            FXMLLoader dashboard_loader = new FXMLLoader(Main.class.getResource("BUFICOM-FRAMES/students-records.fxml"));
            AnchorPane dashboard = dashboard_loader.load();
    ...
    @FXML
    private void add_contribution_button_clicked(){
        try {
            Stage add_contribution_stage = new Stage();
            add_contribution_stage.initModality(Modality.APPLICATION_MODAL);
            FXMLLoader add_contribution_loader = new FXMLLoader(Main.class.getResource("BUFICOM-FRAMES/add-contribution-form.fxml"));
            ...
            add_contribution_control.initialize(this.getOrg_Code(), LoginProcess.getAcademic_year());
    ...
    private void getUserOrgInfo(){
        try {
            String org_code_query = "SELECT * \n" +
                    "FROM `manages` AS m LEFT JOIN `organizations` AS o ON m.`organization_code` = o.`organization_code`\n" +
